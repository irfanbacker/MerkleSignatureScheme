...
class MerkleTree:
        ...
        for level in range(self.levelsCount):
            for position in range(int(self.leavesCount/2**level)):
                if level>0:
                    ...
                    if leftChild != None and rightChild != None: self.tree[(level,position)] = MerkleTree.sha256(leftChild+rightChild)
                    elif (level,position) not in self.tree: self.tree[(level,position)] = None
                elif (level,position) not in self.tree: self.tree[(level,position)] = None
    ...
    def addNode(self, position, nodeValue, isHashed = True):
        if type(nodeValue) is list: nodeValue = MerkleTree.concatenateListToString(nodeValue)
        if not isHashed: nodeValue = MerkleTree.sha256(nodeValue)
        self.tree[position] = nodeValue
    
    def getAuthNodesPosition(self, keyIndex):
        authPos = []
        for level in range(self.levelsCount-1):
            if keyIndex%2 == 0: authPos.append((level,keyIndex+1))
            else: authPos.append((level,keyIndex-1))
